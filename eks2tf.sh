usage()
{ echo "Usage: $0 [-r <resource_type>] [-v <yes|no(default)>]" 1>&2; exit 1;
}
x="no"
p="no"
f="no"
...
rm -rf generated/tf.$mysub-eks
mkdir -p generated/tf.$mysub-eks

s=`echo $mysub`
cd generated/tf.$mysub-eks
rm -rf .terraform
if [ "$f" = "no" ]; then
...
echo "---------------------------------------------------------------------------"
echo "aws2tf output files are in generated/tf.$mysub-eks"
echo "---------------------------------------------------------------------------"
